...
async def parser_worker(worker: Worker, stop_event: asyncio.Event):
    ...
    while not stop_event.is_set():
        ...
        try:
            ...
            # TODO: Figure out a way to send BASE URLS from the seed to workers
            links = extract_links(base_url= "", soup=soup)
            ...
            parsed_data.title = meta_info.get("og:title", title_tag.get_text().strip() if title_tag else 'No Title')
